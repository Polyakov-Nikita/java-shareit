...
public class BookingServiceImplTest extends ServiceTest {
    ...
    @BeforeEach
    public void setUp() {
        bookingService = new BookingServiceImpl(userRepository, itemRepository, bookingMapper, bookingRepository);
    }
    ...
    private void runGetAllBookingsTest(BookingSearchState state, BiConsumer<User, List<Booking>> mockSetup) {
        ...
        // Act
        Comparator<BookingResponse> idComparator = Comparator.comparing(BookingResponse::getId);
        List<BookingResponse> actual = bookingService.getAllUserBookings(booker.getId(), state).stream()
                .sorted(idComparator)
                .toList();

        // Assert
        List<BookingResponse> expected = Stream.of(
    ...
    private void runGetAllUserItemBookingsTest(BookingSearchState state, BiConsumer<User, List<Booking>> mockSetup) {
        ...
        // Act
        Comparator<BookingResponse> idComparator = Comparator.comparing(BookingResponse::getId);
        List<BookingResponse> actual = bookingService.getAllUserItemBookings(booker.getId(), state).stream()
                .sorted(idComparator)
                .toList();
