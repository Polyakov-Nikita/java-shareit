...
public class ItemServiceImpl extends ServiceBase implements ItemService {
    private static final Sort SORT_DESC_END = Sort.by(Sort.Direction.DESC, "end");
    private static final Sort SORT_ASC_START = Sort.by(Sort.Direction.ASC, "start");
    ...
    public ItemServiceImpl(UserRepository userRepository, ItemRepository itemRepository, ItemRequestRepository itemRequestRepository,
    ...
    public ItemResponse createItem(long sharerId, CreateItemRequest request) {
        ...
        ItemRequest itemRequest = getItemRequest(request.getRequestId());
        Item item = itemMapper.toItem(request, owner, itemRequest);
    ...
    private ItemRequest getItemRequest(Long itemRequestId) {
        if (itemRequestId != null) {
            return findItemRequest(itemRequestId);
        }
        return null;
    }
