...
public class ItemServiceImplTest extends ServiceTest {
    ...
    @BeforeEach
    public void setUp() {
        itemService = new ItemServiceImpl(userRepository, itemRepository, itemRequestRepository,
    ...
    public void createItem_ReturnsObject() {
        ...
        // Act
        ItemResponse actual = itemService.createItem(owner.getId(), request);
    ...
    @Test
    public void createItem_WithRequestId_ReturnsObject() {
        // Arrange
        User requestor = buildUser(ID);
        ItemRequest itemRequest = buildItemRequest(ID, requestor, NOW.minusDays(1));
        CreateItemRequest request = buildCreateItemRequest(itemRequest);
        User owner = buildUser(ID);
        whenFound(owner);
        whenFound(itemRequest);
        whenSaveReturns(itemRepository, this::saveItem);
    ...
    @Test
    public void createItem_WithRequestId_AbsentRequest_NotFoundException() {
        // Arrange
        long absentItemRequestId = ID + 1;
        CreateItemRequest request = buildCreateItemRequest(absentItemRequestId);
        User owner = buildUser(ID);
        whenFound(owner);
        whenItemRequestNotFound();

        // Act
        Throwable thrown = Assertions.catchThrowable(() -> itemService.createItem(owner.getId(), request));
    ...
    public void createItem_AbsentOwner_NotFoundException() {
        ...
        // Act
        long absentOwnerId = ID + 1;
        Throwable thrown = Assertions.catchThrowable(() -> itemService.createItem(absentOwnerId, request));
    ...
    public void createComment_NoPastBooking_ForbiddenCommentException() {
        ...
        //Assert
        assertForbiddenCommentException(thrown);
    }
    ...
    public void getAllItems_ReturnsArray() {
        ...
        List<Long> itemIds = getIdsFrom(items);
        ...
        // Assert
        List<GetItemResponse> expected = List.of(
    ...
    public void getAllItems_NoItems_ReturnsEmptyArray() {
        ...
        assertMethodsNotCall(bookingRepository, List.of(
                repository -> repository.findLastBookingsForItems(Mockito.any(), Mockito.any()),
                repository -> repository.findNextBookingsForItems(Mockito.any(), Mockito.any())
        ));
        assertMethodNotCall(commentRepository, repository -> repository.findByItemIdIn(Mockito.any()));
    }
    ...
    public void searchItems_ReturnsArray() {
        ...
        // Assert
        List<ItemResponse> expected = List.of(
    ...
    public void searchItems_EmptyText_ReturnsEmptyArray() {
        ...
        // Assert
        Assertions.assertThat(actual).isEmpty();
        assertMethodNotCall(itemRepository, repository ->
                repository.findAllByAvailableTrueAndNameContainingIgnoreCaseOrDescriptionContainingIgnoreCase(Mockito.any(), Mockito.any()));
    }

    private void assertForbiddenCommentException(Throwable throwable) {
    ...
    private CreateItemRequest buildCreateItemRequest(ItemRequest itemRequest) {
        return CreateItemRequest.builder()
                .name("name")
                .description("description")
                .available(true)
    ...
    private CreateItemRequest buildCreateItemRequest(Long itemRequestId) {
        return CreateItemRequest.builder()
                .name("name")
                .description("description")
                .available(true)
    ...
    private List<Long> getIdsFrom(List<Item> items) {
        return items.stream()
                .map(Item::getId)
                .toList();
    }
